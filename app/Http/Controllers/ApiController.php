                    ...
                    if ($dati['id'] == 0) {
                        ...
                        $statistiche_squadra = DB::connection('pgsql')->select(
                            'SELECT
                                    ...
                                    (SELECT SUM(id) from partite   where (id_squadra_casa = (SELECT id_squadra from giocatori where id_utente = \'' . $id . '\' FETCH NEXT 1 ROWS ONLY) OR id_squadra_ospite = (SELECT id_squadra from giocatori where id_utente = \'' . $id . '\' FETCH NEXT 1 ROWS ONLY)) and id_squadra_vincente != (SELECT TOP 1 id_squadra from giocatori where id_utente = \'' . $id . '\' FETCH NEXT 1 ROWS ONLY)) ) as Gol,
